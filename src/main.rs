...
fn init_scan_config(args: &ArgMatches) -> ScanConfig {
    ...
    let sockaddr: SocketAddr = format!("{target}:0").parse().unwrap_or(SocketAddr::new(
    ...
    let concurrency = args.get_one::<usize>("concurrency").map_or(1000, |c| *c);
    let ports = args.get_one::<String>("ports").map_or_else(
        || parse_ports(&String::from("1-1024")),
        |p| parse_ports(p.as_str()),
    );
    let timeout = args.get_one::<u64>("timeout").map_or(3, |t| *t);
...
fn parse_ports(ports_arg: &str) -> Vec<u16> {
    ...
    if ports_arg.contains(',') {
        ...
        for p in ps {
            ...
            if p.contains('-') {
                let range: Vec<u16> = p
                    .split('-')
                    .map(|x: &str| x.parse::<u16>().unwrap_or_default())
                    .collect();
                ports.extend(range[0]..=range[1]);
            }
            // List item is single port: `x`
            else if let Ok(p) = p.parse::<u16>() {
                ports.push(p);
            }
        }
    }
...
async fn open_port(c: &ScanConfig, port: u16) {
    ...
    let socket_address = SocketAddr::new(c.target_ip, port);

    match tokio::time::timeout(timeout, TcpStream::connect(&socket_address)).await {
        Ok(Ok(_)) => println!("{port}: open"),
        _ => {
            if c.verbose {
                println!("{port}: closed");
            }
